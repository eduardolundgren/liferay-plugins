...
String title = ParamUtil.getString(request, "title_".concat(themeDisplay.getLanguageId()), ParamUtil.getString(request, "titleCurrentValue"));
...
<aui:script>
	...
	<c:if test="<%= calendarBooking == null %>">
		document.<portlet:namespace />fm.<portlet:namespace />title_<%= themeDisplay.getLanguageId() %>.value = '<%= HtmlUtil.escapeJS(title) %>';
	</c:if>
</aui:script>
